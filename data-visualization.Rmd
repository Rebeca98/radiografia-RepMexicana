...
```{r}
transform(delitos_contra_vida, pop.total)
```
...
g1 <- delitos_contra_libertad_seguridad_sexual %>% group_by(estado)%>% 
            ...
            labs(y="Estados", x="número de delitos por cada cien mil habitantes")+ggtitle("Abuso Sexual")+theme(axis.text=element_text(size=7),
...
g2 <- delitos_contra_libertad_seguridad_sexual %>% group_by(estado)%>% 
            ...
            geom_bar(stat='identity', color='gray', fill='red') +
            labs(y="Estados", x="número de delitos por cada cien mil habitantes")+ggtitle("Violación")+theme(axis.text=element_text(size=7),
        axis.title=element_text(size=9,face="bold"))
...
g3 <- delitos_contra_libertad_seguridad_sexual %>% group_by(estado)%>% 
            ...
            geom_bar(stat='identity', color='gray', fill='red') +
            labs(y="Estados", x="número de delitos por cada cien mil habitantes")+ggtitle("Acoso Sexual")+theme(axis.text=element_text(size=7),
        axis.title=element_text(size=9,face="bold"))
...
g4 <- delitos_contra_libertad_seguridad_sexual %>% group_by(estado)%>% 
            ...
            geom_bar(stat='identity', color='gray', fill='red') +
            labs(y="Estados", x="número de delitos por cada cien mil habitantes")+ggtitle("Hostigamiento Sexual")+theme(axis.text=element_text(size=7),
        axis.title=element_text(size=9,face="bold"))
...
g5 <- delitos_contra_libertad_seguridad_sexual %>% group_by(estado)%>% 
            ...
            geom_bar(stat='identity', color='gray', fill='red') +
            labs(y="Estados", x="número de delitos por cada cien mil habitantes")+ggtitle("Corrupción de Menores")+theme(axis.text=element_text(size=7),
        axis.title=element_text(size=9,face="bold"))
```
...
delitos_contra_vida <- delitos_contra_vida %>% 
  dplyr::mutate(prop.Homicidio.doloso = (Homicidio.doloso/pop.total)*100000,
         ...
         prop.Feminicidio = (Feminicidio/pop.total)*1000,
         )
```
...
```{r}
gf <- delitos_contra_vida %>% group_by(estado)%>% 
            ggplot(aes(y=reorder(estado,prop.Feminicidio),x =prop.Feminicidio))+
            geom_bar(stat='identity', color='#F2B8C6', fill='#FC94AF') +
            labs(y="Estados", x="número de feminicidios por cada cien mil habitantes")+ggtitle("Feminicidio")+theme(axis.text=element_text(size=7),
        axis.title=element_text(size=9,face="bold"))
```

```{r}
ggsave(
  filename="plots/feminicidio.png",
  plot = gf
)
```
...
```{r}
newData <- delitos_contra_vida %>% mutate(y=case_when(prop.Feminicidio <= 0.011165 ~ 0.25,
                                           prop.Feminicidio <= 0.015170 ~ 0.50,
                                           prop.Feminicidio >= 0.015170 ~ 0.75
                                           ))
```
...
mapa_feminicidios <- ggplot(mapa,aes(long,lat,group=group, fill=factor(y)))+
  geom_polygon(colour="grey") + coord_quickmap() +scale_fill_manual("tasa de feminicios", values = brewer.pal(5, "RdPu"))+ ggtitle("Feminicidios")+theme(plot.title = element_text(hjust = 0.5))
```
...
create_plot_robos <- function(estado){
  d <- robos %>% filter(estado == estado)
  d <- dplyr::select(d,-c("estado", "pop.total"))
  ...
  g <- ggplot(data=data, aes(x=name, y=values, fill=delitos)) +geom_bar(stat = "identity") +   theme(axis.text.x = element_blank()) + labs(title=paste("Robos en ",estado), y="número de robos",x="delitos")
  ggsave(
...
tipos de robo a nivel nacional
...
```{r}
H1 <- cluster_delitos_libertad_sexual
```


```{r}
sort(table(), decreasing = T)
sort(round(table(cluster_delitos_libertad_sexual[which(km1$cluster==2)])/nrow(cluster_delitos_libertad_sexual$estado)*100,2), decreasing = T)
sort(round(table(cluster_delitos_libertad_sexual[which(km1$cluster==3)])/nrow(cluster_delitos_libertad_sexual$estado)*100,2), decreasing = T)
```
...
```{r}
values_1 <- as.tibble(which(km1$cluster==1))$value
values_2 <- as.tibble(which(km1$cluster==2))$value
values_3 <- as.tibble(which(km1$cluster==3))$value
values_4 <- as.tibble(which(km1$cluster==4))$value
values_5 <- as.tibble(which(km1$cluster==5))$value
```
...
```{r}
plot(cluster_delitos_libertad_sexual$prop.Abuso.sexual,cluster_delitos_libertad_sexual$prop.Hostigamiento.sexual, col=km1$cluster)
points(km1$centers[,c("prop.Abuso.sexual","prop.Hostigamiento.sexual")], col=1:5, pch=16, cex=2)
```
